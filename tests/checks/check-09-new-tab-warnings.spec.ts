...
async function findTargetBlankLinksWithoutWarnings(page: import('@playwright/test').Page): Promise<ViolationDetail[]> {
  const findings = await page.evaluate<LinkWarningAnalysis[], { warningPatternSource: string; warningPatternFlags: string }>(({ warningPatternSource, warningPatternFlags }) => {
