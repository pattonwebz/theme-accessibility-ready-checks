...
test.describe('check-15: hover/focus content', () => {
  for (const templateName of ACTIVE_TEMPLATES) {
    test(`hover-focus-1 — hover/focus content is dismissable on ${templateName}`, async ({ page, templateUrl }, testInfo: TestInfo) => {
    ...
    test(`hover-focus-2 — hover/focus content is hoverable and persistent on ${templateName}`, async ({ page, templateUrl }, testInfo: TestInfo) => {
      test.skip(testInfo.project.name !== 'desktop', 'Hover/focus checks are desktop-only');
