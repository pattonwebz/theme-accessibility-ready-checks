...
LOCATIONS=$($WP menu location list --fields=location --format=csv 2>/dev/null | tail -n +2)
...
if [ -z "$LOCATIONS" ]; then
  ...
  if $WP menu list --fields=name --format=csv 2>/dev/null | tail -n +2 | grep -Fxq "$MENU_NAME"; then
    ...
    HOME_MENU_ID=$($WP menu item add-post "$MENU_NAME" "$HOME_PAGE_ID" --title="Home" --porcelain)
    ABOUT_ID=$($WP menu item add-post "$MENU_NAME" "$PARENT_PAGE_ID" --title="About" --porcelain)
    OUR_TEAM_ID=$($WP menu item add-custom "$MENU_NAME" "Our Team" "#" --parent-id="$ABOUT_ID" --porcelain)
    SERVICES_ID=$($WP menu item add-post "$MENU_NAME" "$MARKUP_PAGE_ID" --title="Services" --porcelain)
    ACCESSIBILITY_ID=$($WP menu item add-post "$MENU_NAME" "$PATTERNS_PAGE_ID" --title="Accessibility" --parent-id="$SERVICES_ID" --porcelain)
    WCAG_ID=$($WP menu item add-custom "$MENU_NAME" "WCAG Compliance" "#" --parent-id="$ACCESSIBILITY_ID" --porcelain)
    BLOG_ID=$($WP menu item add-post "$MENU_NAME" "$BLOG_PAGE_ID" --title="Blog" --porcelain)
