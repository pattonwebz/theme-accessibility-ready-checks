...
import { test, expect, ACTIVE_TEMPLATES } from '../../../../tests/helpers/fixtures';
import { recordResult } from '../../../../tests/helpers/result-collector';
    ...
    const controls = Array.from(document.querySelectorAll('select, textarea, input'))
      .filter((element): element is HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement => {
        ...
        if (element instanceof HTMLInputElement) {
          return ['text', 'email', 'search'].includes(element.type) && !element.readOnly;
        }
